...
{
  pinMode(_irq, INPUT);
  // KGF: hacking to work for bare ATmega328p rather than Arduino Uno (PB6 pin)
  //pinMode(_reset, OUTPUT);
  // Set pin PB6 as output
  DDRB |= 1 << DDB6;
}
...
void Adafruit_PN532::begin() {
  if (_usingSPI) {
    ...
    // Modified code for ATmega328p using PB6 pin
    // PORTC |= 1 << PC2; // Write 1 to PB6
    // PORTC &= ~(1 << PC2); // Write 0 to PB6
    // _delay_ms(400); 
    // PORTC |= 1 << PC2; // Write 1 to PB6
    // _delay_ms(10);

    
    PORTD |= 1 << PD4; // Write 1 to PD4
    PORTD &= ~(1 << PD4); // Write 0 to PD4
    _delay_ms(400);
    PORTD |= 1 << PD4; // Write 1 to PD4
    _delay_ms(10);
